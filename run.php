...
$maxquery = "SELECT MAX(TimeRan) AS maximum FROM Progress WHERE UserID = '$userID'";
...
echo "<div class='container-fluid padding postloginpage'>
        <div class='row aboutwelcome text-center'>
            <div class='col-12 myabouttext'>
                <h1 class='display-4'>Running Statistics <i class='fas fa-chart-line fa-sm'></i></h1>
                <hr>
                <h3>Your current longest run is: $maximum minutes</h3>
                <div class='col-12'>
                </div>
            </div>
        </div>
   ...
   // Retrieving and displaying time ran for results
   $runquery = "SELECT TimeRan, DateDone FROM Progress WHERE UserID = '$userID'";

   $runresult = $conn->query($runquery);
   ...
   while($row = $runresult->fetch_assoc()){
       ...
       <tr>
            <th style = 'text-align:center'><h3>Time Ran</h3></th>
            <th style = 'text-align:center'><h3>Date Achieved</h3></th>
       </tr>
       <tr>
            <td style = 'text-align:center'><h4>$run minutes</h4></td>

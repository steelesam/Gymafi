...
$maxquery = "SELECT MAX(OverheadPress) AS maximum FROM Progress WHERE UserID = '$userID'";
...
echo "<div class='container-fluid padding postloginpage'>
        <div class='row aboutwelcome text-center'>
            <div class='col-12 myabouttext'>
                <h1 class='display-4'>Overhead Press Statistics <i class='fas fa-chart-line fa-sm'></i></h1>
                <hr>
   ...
   // Retrieving and displaying ohp results
   $ohpquery = "SELECT OverheadPress, DateDone FROM Progress WHERE UserID = '$userID'";

   $ohpresult = $conn->query($ohpquery);

   if(!$ohpresult){
       echo $conn->error;
   }

   while($row = $ohpresult->fetch_assoc()){

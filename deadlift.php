...
while($row = $result->fetch_assoc()){
 ...
 $maxquery = "SELECT MAX(Deadlift) AS maximum FROM Progress WHERE UserID = '$userID'";
...
echo "<div class='container-fluid padding postloginpage'>
        <div class='row aboutwelcome text-center'>
            <div class='col-12 myabouttext'>
                <h1 class='display-4'>Deadlift Statistics <i class='fas fa-chart-line fa-sm'></i></h1>
                <hr>
   ...
   // Retrieving and displaying deadlift results
   $deadliftquery = "SELECT Deadlift, DateDone FROM Progress WHERE UserID = '$userID'";

   $deadliftresult = $conn->query($deadliftquery);

   if(!$deadliftresult){
       echo $conn->error;
   }

   while($row = $deadliftresult->fetch_assoc()){

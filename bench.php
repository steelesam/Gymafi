...
while($row = $result->fetch_assoc()){
    ...
    $maxquery = "SELECT MAX(BenchPress) AS maximum FROM Progress WHERE UserID = '$userID'";
...
echo "<div class='container-fluid padding postloginpage'>
        <div class='row aboutwelcome text-center'>
            <div class='col-12 myabouttext'>
                <h1 class='display-4'>Bench Press Statistics <i class='fas fa-chart-line fa-sm'></i></h1>
                <hr>
    ...
    // Retrieving and displaying bench results
    $benchquery = "SELECT BenchPress, DateDone FROM Progress WHERE UserID = '$userID'";

    $benchresult = $conn->query($benchquery);

    if(!$benchresult){
        echo $conn->error;
    }

    while($row = $benchresult->fetch_assoc()){
        ...
        <tr>
            <th style = 'text-align:center'><h3>Bench Press</h3></th>
